...
#import <tgmath.h>
#import "LottieView.h"
#define VioScreenW [UIScreen mainScreen].bounds.size.width
#define VioScreenH [UIScreen mainScreen].bounds.size.height
#define VioSch                             (VioScreenH / 667.0)
#define VioScw                             (VioScreenW / 375.0)
...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"LottieViewManager needs to be accessed on the main thread.");
...
@interface LottieViewManagerRoundedButton : UIButton
@end


@implementation LottieViewManager

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated frame:(CGRect)frame animationJson:(NSString*)animationJson{
    LottieViewManager *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud showAnimated:animated];
    LottieView *hudCustomView = [[LottieView alloc] initWithFrame:frame animationJson:animationJson];
    ...
    hud.bezelView.color = [UIColor colorWithWhite:0.0f alpha:0.0000001f];
    return hud;
}

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated  animationJson:(NSString*)animationJson{
    LottieViewManager *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud showAnimated:animated];
    LottieView *hudCustomView = [[LottieView alloc] initWithFrame:CGRectMake(0, 0, 100*VioScw, 100*VioScw) animationJson:animationJson];
    hud.mode = LottieViewManagerModeCustomView;
    hud.customView = hudCustomView;
    hud.bezelView.style = LottieViewManagerBackgroundStyleSolidColor;
    hud.bezelView.color = [UIColor colorWithWhite:0.0f alpha:0.0000001f];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LottieViewManager *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LottieViewManager *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (LottieViewManager *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = LottieViewManagerAnimationFade;
    _mode = LottieViewManagerModeIndeterminate;
    _margin = 20.f;
    _opacity = 1.f;
    ...
    _contentColor = isLegacy ? [UIColor clearColor] : [UIColor clearColor];
...
- (void)animateIn:(BOOL)animatingIn withType:(LottieViewManagerAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LottieViewManagerAnimationZoom) {
        type = animatingIn ? LottieViewManagerAnimationZoomIn : LottieViewManagerAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LottieViewManagerAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LottieViewManagerAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LottieViewManagerAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LottieViewManagerAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    LottieViewManagerCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LottieViewManagerDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = LottieViewManagerBackgroundStyleSolidColor;
    ...
    UIButton *button = [LottieViewManagerRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    LottieViewManagerMode mode = self.mode;
    if (mode == LottieViewManagerModeIndeterminate) {
    ...
    else if (mode == LottieViewManagerModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LottieViewManagerModeDeterminate || mode == LottieViewManagerModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == LottieViewManagerModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LottieViewManagerModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LottieViewManagerModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LottieViewManager class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LottieViewManager class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[LottieViewManager class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LottieViewManager class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[LottieViewManager class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LottieViewManager class]]];
#endif
...
- (void)setMode:(LottieViewManagerMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LottieViewManagerModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = LottieViewManagerBackgroundStyleBlur;
            ...
            _style = LottieViewManagerBackgroundStyleSolidColor;
            _color = [UIColor clearColor];
        }
...
- (void)setStyle:(LottieViewManagerBackgroundStyle)style {
    if (style == LottieViewManagerBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = LottieViewManagerBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    LottieViewManagerBackgroundStyle style = self.style;
    if (style == LottieViewManagerBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LottieViewManagerBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation LottieViewManager (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LottieViewManager allHUDsForView:view];
    for (LottieViewManager *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LottieViewManagerCompletionBlock)completion {
...
- (BOOL)dimBackground {
    ...
    UIColor *dimmedColor =  [UIColor clearColor];
    return backgroundView.style == LottieViewManagerBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = LottieViewManagerBackgroundStyleSolidColor;
...
@implementation LottieViewManagerRoundedButton
...
- (void)setHighlighted:(BOOL)highlighted {
    ...
    self.backgroundColor = highlighted ? [UIColor clearColor] : [UIColor clearColor];
}
-(void)dealloc{
